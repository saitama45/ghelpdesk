...
- Per-milestone Excel import: the Gantt milestone header's **Import** button → `POST projects/{project}/milestone-import` (`ProjectTaskController::importMilestone`, gated like `+ Activity` via `ProjectPlanAccess::canAddActivity`). Workbook rules live in `app/Services/MilestoneActivityImportService.php` (template = `GET …/milestone-import-template`; worked example `References/gantt_milestone_import_template.xlsx`). Columns: Code, Parent Code, Name, Lead Time, ONE Depends On Code, Run Parallel, Responsible Email, Acceptance Criteria, Remarks. Validate-all-then-write (any error → 422, nothing saved); upserts by name (activity in milestone / sub-task under parent); never deletes; then `reschedule()` + board sync.
- Bulk delete inside a milestone: red selection boxes (header = every row under the milestone; activity with sub-tasks = its sub-tasks only; other rows = themselves) → header "Delete N" → `POST projects/{project}/milestone-tasks/bulk-destroy` (`ProjectTaskController::bulkDestroyMilestoneTasks`). Every id must belong to that project + milestone and pass `canDeleteTask`; the milestone is never deleted (its `project_milestones` record is created if missing), and `groupedTasks` in `ProjectGantt.vue` renders milestones from those records even with zero rows. An activity id is only sent when all of its sub-tasks are ticked.

    ...
    <form @submit.prevent="submit" class="space-y-8">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 p-10 border border-gray-100 relative overflow-hidden dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="relative z-10">
                <h2 class="text-3xl font-black text-gray-900 tracking-tight mb-10 flex items-center dark:text-gray-100">
                ...
                <div v-if="isPublic" class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Your Name</label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Your Email</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Entity/Dept.</label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">POS Request Type</label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Launch Date</label>
                ...
                <div class="mt-12 p-8 rounded-[2.5rem] border-2 transition-all" 
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p v-else class="text-[10px] text-gray-400 font-bold uppercase mt-1 italic dark:text-gray-400">Tag specific stores or apply to all</p>
                        </div>
                    ...
                    <div v-if="!isAllStores" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 dark:text-gray-400">Filter By Cluster</label>
                                <select v-model="selectedCluster"
                                        class="w-full bg-white border-2 border-gray-100 rounded-2xl px-5 py-4 text-sm font-bold focus:border-indigo-500 focus:ring-0 transition-all outline-none shadow-sm dark:bg-gray-800 dark:border-gray-700">
                                    <option value="">All Clusters</option>
                                    <option v-for="cluster in clusterOptions" :key="cluster" :value="cluster">{{ cluster }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 mb-2 dark:text-gray-400">Filter By Brand</label>
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            <div class="flex-1 relative">
                                <svg class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                </svg>
                                <input v-model="storeSearch" type="text" placeholder="Search stores by code, name, cluster, or brand..."
                                       class="w-full bg-white border-2 border-gray-100 rounded-2xl pl-14 pr-6 py-4 text-sm font-bold focus:border-indigo-500 focus:ring-0 transition-all outline-none shadow-sm dark:bg-gray-800 dark:border-gray-700">
                            </div>
                            
                            <div class="flex-[2] bg-white border-2 rounded-2xl p-4 min-h-[60px] flex flex-wrap gap-2 shadow-sm transition-colors dark:bg-gray-800"
                                ...
                                <span v-if="hiddenSelectedStoreCount > 0"
                                      class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-black uppercase tracking-widest dark:bg-gray-800 dark:text-gray-300">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <p class="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-gray-400">
                            ...
                            <button type="button" @click="clearStoreFilters"
                                    class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest bg-white text-gray-500 border border-gray-200 hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">
                                Clear Filters
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                            <button v-for="s in filteredStores" :key="s.id" type="button"
                                    ...
                                    class="px-4 py-3 rounded-xl text-[10px] font-black uppercase text-left transition-all border border-gray-100 shadow-sm truncate dark:border-gray-700">
                                {{ s.code }}
                            </button>
                        </div>
                        <p v-if="filteredStores.length === 0" class="text-center text-xs font-bold text-gray-400 italic py-6 dark:text-gray-400">
                            No stores matched the selected cluster, brand, and search filters.
                        </p>
                    </div>
                    <div v-else class="text-center py-12 bg-white rounded-3xl border-2 border-dashed border-indigo-100 dark:bg-gray-800">
        ...
        <div v-if="form.request_type_id && useSchema" class="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 p-10 border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center mb-8">
                ...
                <div>
                    <h2 class="text-2xl font-black text-gray-900 tracking-tight dark:text-gray-100">{{ selectedRequestType?.name }} Details</h2>
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 dark:text-gray-400">Dynamic fields configured by admin</p>
                </div>
            </div>
            <DynamicFormRenderer
                :fields="schemaFields"
        ...
        <div v-else-if="form.request_type_id" class="bg-white rounded-[2.5rem] shadow-2xl shadow-gray-200/50 p-10 border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center justify-between mb-10">
                <div class="flex items-center">
                    ...
                    <div>
                        <h2 class="text-3xl font-black text-gray-900 tracking-tight dark:text-gray-100">Line Item Details</h2>
                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 dark:text-gray-400">Configure product pricing and technical specs</p>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                >
                    <div v-for="(row, index) in form.details" :key="index" 
                         ...
                         class="relative rounded-[2rem] p-8 border-2 hover:border-indigo-100 hover:bg-white hover:shadow-2xl hover:shadow-gray-200/40 transition-all group dark:hover:bg-gray-700">
                        
                        <div class="flex justify-between items-start mb-8">
                            <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="space-y-2">
                                    <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Product Name</label>
                                ...
                                <div class="space-y-2">
                                    <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">POS Display Name (Alias)</label>
                                ...
                                <div class="space-y-2">
                                    <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Item Code</label>
                                    <input v-model="row.item_code" placeholder="SKU/SAP Code..."
                                           class="w-full bg-white border-2 border-gray-100 rounded-xl px-5 py-3.5 text-xs font-mono font-black focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all outline-none shadow-sm dark:bg-gray-800 dark:border-gray-700">
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-6 mb-8 p-6 bg-white/50 rounded-3xl border border-gray-100 dark:border-gray-700">
                            <div class="space-y-2">
                                ...
                                <select v-model="row.price_type" required 
                                        class="w-full bg-white border-2 border-gray-100 rounded-xl px-4 py-3 text-[10px] font-black focus:border-indigo-500 focus:ring-0 transition-all outline-none dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs dark:text-gray-400">₱</span>
                                    <input v-model="row.price_amount" type="number" step="0.01" placeholder="0.00"
                                           class="w-full bg-white border-2 border-gray-100 rounded-xl pl-8 pr-4 py-3 text-sm font-black focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none dark:bg-gray-800 dark:border-gray-700">
                                </div>
                            </div>
                            <div class="space-y-2 text-center">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">SC</label>
                                <input v-model="row.sc" class="w-full bg-white border-2 border-gray-100 rounded-xl px-4 py-3 text-sm font-bold text-center focus:border-indigo-500 focus:ring-0 dark:bg-gray-800 dark:border-gray-700">
                            </div>
                            <div class="space-y-2 text-center">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Tax %</label>
                                <input v-model="row.local_tax" class="w-full bg-white border-2 border-gray-100 rounded-xl px-4 py-3 text-sm font-bold text-center focus:border-indigo-500 focus:ring-0 dark:bg-gray-800 dark:border-gray-700">
                            </div>
                            <div class="space-y-2 text-center flex flex-col items-center">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] dark:text-gray-400">Mgr's Meal</label>
                                <div class="mt-1 flex flex-col items-center">
                                    <button type="button" 
                                        ...
                                        <span :class="row.mgr_meal ? 'translate-x-5' : 'translate-x-0'"
                                              class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-300 ease-in-out dark:bg-gray-800"></span>
                                    </button>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                            <div class="space-y-2 relative">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Category</label>
                                <input v-model="row.category" placeholder="Search or type category..."
                                       @focus="activeSuggest = { index, type: 'category' }"
                                       class="w-full bg-white border-2 border-gray-100 rounded-xl px-5 py-3 text-sm font-bold focus:border-indigo-500 focus:ring-0 outline-none dark:bg-gray-800 dark:border-gray-700">
                                <div v-if="activeSuggest.index === index && activeSuggest.type === 'category' && filterSuggestions('category', row.category).length" 
                                     class="absolute z-20 left-0 right-0 top-full mt-1 bg-white border border-gray-100 shadow-2xl rounded-xl overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <div class="space-y-2 relative">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Sub Category</label>
                            ...
                            <div class="space-y-2">
                                ...
                                <input v-model="row.validity_date" type="date" 
                                       class="w-full bg-white border-2 border-gray-100 rounded-xl px-5 py-2.5 text-sm font-bold focus:border-indigo-500 focus:ring-0 outline-none dark:bg-gray-800 dark:border-gray-700">
                            </div>
                            <div class="space-y-2">
                                <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Printer Selection</label>
                        ...
                        <div class="space-y-2">
                            <label class="block text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1 dark:text-gray-400">Remarks & Mechanics</label>
                            <textarea v-model="row.remarks_mechanics" rows="2" placeholder="Describe mechanics..."
                                      class="w-full bg-white border-2 border-gray-100 rounded-2xl px-5 py-4 text-sm font-medium focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 transition-all outline-none shadow-sm dark:bg-gray-800 dark:border-gray-700"></textarea>
                        </div>
            ...
            <div class="mt-10 flex justify-center">
                <button type="button" @click="addRow" class="group flex items-center px-10 py-4 bg-gray-50 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl text-xs font-black uppercase tracking-widest transition-all border-2 border-dashed border-gray-200 hover:border-indigo-200 dark:bg-gray-900/50 dark:text-gray-400 dark:border-gray-700">
        ...
        <div class="flex justify-end items-center space-x-8 pb-12">
            <Link v-if="!isPublic" :href="route('pos-requests.index')" class="text-xs font-black text-gray-400 uppercase tracking-[0.3em] hover:text-gray-900 transition-all underline decoration-2 decoration-transparent hover:decoration-indigo-500 underline-offset-8 dark:text-gray-400">Discard Request</Link>
            ...
            <div v-if="showConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h3 class="text-lg font-black text-gray-900 dark:text-gray-100">Submit POS Request</h3>
                            <p class="text-sm text-gray-500 mt-0.5 dark:text-gray-300">Please review all details before confirming.</p>
                        </div>
                    </div>
                    <p class="text-sm text-gray-600 mb-8 dark:text-gray-300">Are you sure you want to submit this POS request? Once submitted, it will be sent for processing.</p>

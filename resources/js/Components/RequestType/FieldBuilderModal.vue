                ...
                <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col border border-gray-100 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="flex items-start justify-between p-7 border-b border-gray-100 shrink-0 dark:border-gray-700">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-0.5 font-medium dark:text-gray-300">{{ requestType.name }}</p>
                        </div>
                    ...
                    <div class="flex border-b border-gray-100 px-7 shrink-0 dark:border-gray-700">
                        <button
                            ...
                            :class="[
                                ...
                                activeTab === tab.key ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                            ]"
                        >
                            {{ tab.label }}
                    ...
                    <div class="flex-1 overflow-y-auto p-7 space-y-4">
                        ...
                        <template v-if="activeTab === 'fields' || activeTab === 'approver'">
                            ...
                            <div v-if="activeFields.length" class="space-y-2">
                                <div
                                    ...
                                    :key="idx"
                                    class="flex items-center gap-3 p-3 rounded-2xl border border-gray-100 bg-gray-50 group dark:bg-gray-900/50 dark:border-gray-700"
                                >
                                    <!-- Move -->
                            ...
                            <div v-if="editingField" class="mt-2 p-5 bg-indigo-50 rounded-2xl border border-indigo-100 space-y-4 dark:bg-indigo-900/20 dark:border-indigo-800">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input v-model="editingField.label" type="text" placeholder="e.g. Vendor Name" class="w-full rounded-xl border-gray-200 text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-800 dark:border-gray-700" />
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wider mb-1 dark:text-gray-300">Key *</label>
                                        <input v-model="editingField.key" type="text" placeholder="vendor_name" class="w-full rounded-xl border-gray-200 text-sm font-mono focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-800 dark:border-gray-700" />
                                    </div>
                                </div>
                                <!-- Type selector -->
                                <div>
                                        ...
                                        <button
                                            ...
                                            :class="[
                                                ...
                                                editingField.type === ft.value
                                                    ...
                                                    : 'bg-white text-gray-600 border border-gray-200 hover:border-indigo-300 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:border-indigo-500'
                                            ]"
                                        >{{ ft.label }}</button>
                                    </div>
                                </div>
                                ...
                                <div v-if="HAS_OPTIONS.includes(editingField.type)" class="space-y-3">
                                                ...
                                                <div v-if="editingField.has_option_approvers" class="mt-3 space-y-3 border-t border-gray-100 pt-3 dark:border-gray-700">
                                                    ...
                                                    <div v-if="opt.has_custom_approval_matrix" class="space-y-3">
                                                        <div>
                                                            ...
                                                            <div class="flex items-center space-x-3 bg-gray-50 rounded-2xl p-1 border border-gray-200 dark:bg-gray-900/50 dark:border-gray-700">
                                                                <button
                                                                    type="button"
                                    ...
                                    <template v-else>
                                        <div>
                                            ...
                                            <select v-model="editingField._dependsOnField" class="w-full rounded-xl border-gray-200 text-xs bg-white focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <template v-else>
                            ...
                            <template v-if="schema.has_items">
                                ...
                                <!-- Column list -->
                                <div>
                                    <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wider mb-2 dark:text-gray-300">Columns</label>
                                    <div v-if="schema.items_columns.length" class="space-y-2 mb-3">
                                        <div v-for="(col, idx) in schema.items_columns" :key="idx"
                                            class="flex items-center gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100 group dark:bg-gray-900/50 dark:border-gray-700">
                                            <div class="flex-1 min-w-0">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex flex-wrap gap-1.5">
                                            <button v-for="t in ITEM_COL_TYPES" :key="t" type="button" @click="editingItemCol.type = t"
                                                :class="['px-3 py-1.5 rounded-xl text-xs font-bold transition-colors', editingItemCol.type === t ? 'bg-teal-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:border-teal-300 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:border-teal-500']">
                                                {{ typeLabel(t) }}
                                            </button>
                                        </div>
                                    </div>

        ...
        <div class="space-y-6">
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1 dark:text-gray-300">Sub-Unit</label>
                        <select v-model="filterForm.sub_unit" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm dark:border-gray-600">
                            <option value="all">All Sub-Units</option>
                            <option v-for="unit in subUnits" :key="unit" :value="unit">{{ unit }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1 dark:text-gray-300">Assignee</label>
                        <select v-model="filterForm.user_id" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm dark:border-gray-600">
                            <option value="all">All Assignees</option>
                            <option v-for="user in users" :key="user.id" :value="user.id">{{ user.name }}</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1 dark:text-gray-300">Start Date</label>
                        <input type="date" v-model="filterForm.start_date" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm dark:border-gray-600">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-700 uppercase mb-1 dark:text-gray-300">End Date</label>
                        <input type="date" v-model="filterForm.end_date" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm dark:border-gray-600">
                    </div>
                    <div class="md:col-span-4 flex justify-end space-x-2">
                        ...
                        <button @click="exportPDF" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-md text-sm font-medium flex items-center shadow-sm transition-colors border border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:bg-gray-700">
            ...
            <div v-if="reportData.length > 0" class="grid grid-cols-1 gap-6">
                <div v-for="user in reportData" :key="user.user_id" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden dark:bg-gray-800 dark:border-gray-700">
                    <!-- Card Header -->
                    <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap justify-between items-center gap-4 dark:bg-gray-900/50 dark:border-gray-700">
                        <div>
                            <h3 class="text-lg font-black text-gray-900 uppercase tracking-tight dark:text-gray-100">{{ user.user_name }}</h3>
                            <p class="text-xs text-gray-500 font-bold uppercase dark:text-gray-300">{{ user.sub_unit || 'No Sub-Unit' }}</p>
                        </div>
                        <div class="flex gap-6">
                            <div class="text-right">
                                <span class="text-2xl font-black text-blue-600">{{ user.total_tickets }}</span>
                                <p class="text-[10px] text-gray-400 font-black uppercase dark:text-gray-400">Total Tickets</p>
                            </div>
                            <div class="text-right">
                                <span class="text-2xl font-black text-emerald-600">{{ user.closed_tickets }}</span>
                                <p class="text-[10px] text-gray-400 font-black uppercase dark:text-gray-400">Closed/Resolved</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-end">
                                <h4 class="text-sm font-black text-gray-700 uppercase tracking-widest flex items-center dark:text-gray-300">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-end">
                                ...
                                <div class="text-right" v-if="user.survey.total > 0">
                                    ...
                                    <span class="text-xs text-gray-400 ml-1 dark:text-gray-400">/ 4</span>
                                </div>
                            </div>

                            <div v-if="user.survey.total > 0">
                                <!-- Avg rating label bar -->
                                <div class="flex items-center justify-between mb-3 p-2 rounded-lg bg-gray-50 border border-gray-100 dark:bg-gray-900/50 dark:border-gray-700">
                                    <span class="text-xs font-black uppercase" :class="getRatingColor(user.survey.avg_rating)">
                                        {{ getRatingLabel(user.survey.avg_rating) }}
                                    </span>
                                    <span class="text-[10px] text-gray-400 font-bold dark:text-gray-400">{{ user.survey.total }} survey{{ user.survey.total !== 1 ? 's' : '' }} / {{ user.total_tickets }} tickets</span>
                                </div>
                                ...
                                <div v-if="user.survey.feedbacks && user.survey.feedbacks.length > 0" class="mt-4 space-y-2">
                                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-1 dark:text-gray-400 dark:border-gray-700">Recent Feedback</p>
                                    <div v-for="(feedback, fIdx) in user.survey.feedbacks.slice(0, 3)" :key="fIdx" class="bg-gray-50 p-2 rounded border border-gray-100 text-xs dark:bg-gray-900/50 dark:border-gray-700">
                                        <div class="flex justify-between items-start mb-1">
                                            <span class="font-bold">{{ getRatingEmoji(feedback.rating) }}</span>
                                            <span class="text-[9px] text-gray-400 dark:text-gray-400">{{ feedback.date }}</span>
                                        </div>
                                        <p class="text-gray-600 italic dark:text-gray-300">"{{ feedback.text }}"</p>
                                    </div>
                                </div>
                            </div>

                            <div v-else class="flex flex-col items-center justify-center h-24 bg-gray-50 rounded-lg border border-dashed border-gray-200 dark:bg-gray-900/50 dark:border-gray-700">
                                <p class="text-2xl mb-1">📋</p>
                                <p class="text-xs text-gray-400 font-bold uppercase dark:text-gray-400">No surveys yet</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Empty State -->
            <div v-else class="bg-white p-12 rounded-lg shadow-sm border border-gray-200 text-center dark:bg-gray-800 dark:border-gray-700">
                <ChartBarIcon class="w-12 h-12 text-gray-300 mx-auto mb-4" />
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">No performance data found</h3>
                <p class="text-gray-500 dark:text-gray-300">Try adjusting your filters or date range.</p>
            </div>
        </div>
    </AppLayout>
</template>

                ...
                >
                    ...
                    <template #header>
                        <tr class="bg-gray-50/80 backdrop-blur-sm dark:bg-gray-800/80">
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest dark:text-slate-300">Code</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest dark:text-slate-300">Request Type</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest dark:text-slate-300">Systems</th>
                            <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-widest dark:text-slate-300">Approvals</th>
                            ...
                            >
                                ...
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition-colors duration-300 dark:text-gray-100">{{ type.name }}</div>
                                            <div class="text-[10px] text-gray-400 font-medium truncate max-w-[150px] dark:text-gray-400" v-if="type.cc_emails">CC: {{ type.cc_emails }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-center">
                                    <span v-if="type.approval_levels > 0" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-black bg-orange-50 text-orange-700 border border-orange-100 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800">
                                        {{ type.approval_levels }} Level{{ type.approval_levels > 1 ? 's' : '' }}
                                    </span>
                                    <span v-else class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black bg-gray-100 text-gray-400 border border-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700">
                                        No Approval
                                    </span>
                                </td>
                                <td class="px-6 py-5 whitespace-nowrap">
                                    <span :class="type.is_active ? 'bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800' : 'bg-rose-100 text-rose-800 border border-rose-200 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800'" 
                                ...
                                <td class="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end items-center space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                                        ...
                                        <div class="flex flex-col items-end space-y-1">
                                            <span v-if="type.form_schema?.fields?.length" class="text-[9px] font-black text-teal-700 bg-teal-50 border border-teal-200 rounded-full px-2 py-0.5 whitespace-nowrap dark:bg-teal-900/30 dark:text-teal-400 dark:border-teal-800">
                                                {{ type.form_schema.fields.length }} fields
                                            </span>
                                            <span v-if="type.form_schema?.approver_fields?.length" class="text-[9px] font-black text-orange-700 bg-orange-50 border border-orange-200 rounded-full px-2 py-0.5 whitespace-nowrap dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800">
                                                {{ type.form_schema.approver_fields.length }} approver fields
                                            </span>
                                            <span v-if="countCheckboxApprovalOverrides(type) > 0" class="text-[9px] font-black text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-full px-2 py-0.5 whitespace-nowrap dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800">
                                                {{ countCheckboxApprovalOverrides(type) }} checkbox override{{ countCheckboxApprovalOverrides(type) !== 1 ? 's' : '' }}
                                            </span>
                                            <span v-if="type.form_schema?.has_items && type.form_schema?.items_columns?.length" class="text-[9px] font-black text-purple-700 bg-purple-50 border border-purple-200 rounded-full px-2 py-0.5 whitespace-nowrap dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800">
                ...
                <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl p-8 border border-gray-100 transform transition-all dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex justify-between items-center mb-8">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1 dark:text-gray-300">Configure system type, approvals, and notifications.</p>
                        </div>
                    ...
                    <form @submit.prevent="submitForm" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 ml-1 dark:text-gray-400">Unique Code</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 ml-1 dark:text-gray-400">Approval Levels</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 ml-1 dark:text-gray-400">Display Name</label>
                            <input v-model="form.name" type="text" required placeholder="e.g. Master Data Update"
                                   class="block w-full px-4 py-3 bg-gray-50 border-transparent rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white focus:border-transparent text-sm font-bold transition-all duration-300 dark:bg-gray-900/50">
                        </div>

                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1 dark:text-gray-400">System Type</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 ml-1 dark:text-gray-400">CC Email Notifications (one per line)</label>
                            <textarea v-model="form.cc_emails" rows="3" placeholder="email1@example.com&#10;email2@example.com"
                                      class="block w-full px-4 py-3 bg-gray-50 border-transparent rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:bg-white focus:border-transparent text-sm font-medium transition-all duration-300 custom-scrollbar dark:bg-gray-900/50"></textarea>
                            <p class="text-[10px] text-gray-400 mt-2 ml-1 italic dark:text-gray-400">These addresses will be notified via CC on ticket updates.</p>
                        </div>

                        <div v-if="form.approval_levels > 0" class="space-y-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-1 ml-1 dark:text-gray-400">Approval Matrix</label>
                                    <p class="text-xs text-gray-500 ml-1 dark:text-gray-300">Assign one or more approvers for each approval level.</p>
                                </div>
                        ...
                        <div v-if="isEditing" class="flex items-center p-4 bg-gray-50 rounded-2xl border border-gray-100 dark:bg-gray-900/50 dark:border-gray-700">

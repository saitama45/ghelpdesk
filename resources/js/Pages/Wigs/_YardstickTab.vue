    ...
    <div class="space-y-8">
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                <h2 class="text-sm font-black uppercase tracking-wider text-gray-700">Performance Standards</h2>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="(s, i) in form.standards" :key="i" class="align-top hover:bg-gray-50/60">
                            <template v-if="editing">
                                <td class="px-2 py-2"><textarea v-model="s.general" rows="2" class="w-40 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2"><textarea v-model="s.specific" rows="2" class="w-48 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2"><textarea v-model="s.rating_4" rows="2" class="w-44 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2"><textarea v-model="s.rating_3" rows="2" class="w-44 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2"><textarea v-model="s.rating_2" rows="2" class="w-44 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2"><textarea v-model="s.rating_1" rows="2" class="w-44 text-xs border-gray-200 rounded"></textarea></td>
                                <td class="px-2 py-2 text-center">
                                    <button @click="form.standards.splice(i, 1)" class="text-red-500 hover:text-red-700 text-xs font-bold">✕</button>
                                </td>
                            </template>
                            <template v-else>
                                <td class="px-3 py-2 font-bold text-gray-800">{{ s.general }}</td>
                                <td class="px-3 py-2 text-gray-600">{{ s.specific }}</td>
                                <td class="px-3 py-2 text-gray-600 bg-green-50/40">{{ s.rating_4 }}</td>
                                <td class="px-3 py-2 text-gray-600">{{ s.rating_3 }}</td>
                                <td class="px-3 py-2 text-gray-600">{{ s.rating_2 }}</td>
                                <td class="px-3 py-2 text-gray-600 bg-red-50/40">{{ s.rating_1 }}</td>
                            </template>
                        </tr>
                    </tbody>
                </table>
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-100">
                <h2 class="text-sm font-black uppercase tracking-wider text-gray-700">Quarterly Guideline</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 p-4">
                <div v-for="(g, i) in form.guidelines" :key="i" class="border border-gray-100 rounded-lg p-3 bg-gray-50/50">
                    <p class="text-xs font-black text-blue-600">Q{{ g.quarter }}</p>
                    <template v-if="editing">
                        <input v-model="g.value_name" placeholder="Value" class="w-full mt-1 text-xs border-gray-200 rounded" />
                        <textarea v-model="g.description" rows="2" placeholder="Description" class="w-full mt-1 text-xs border-gray-200 rounded"></textarea>
                    </template>
                    <template v-else>
                        <p class="text-sm font-bold text-gray-800 mt-0.5">{{ g.value_name }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ g.description }}</p>
                    </template>
                </div>
            </div>
        </section>
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                <h2 class="text-sm font-black uppercase tracking-wider text-gray-700">TRACK Questions &amp; Guide Questions</h2>
            ...
            <div class="divide-y divide-gray-100">
                <div v-for="(v, i) in form.values" :key="i" class="p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1">
                            <template v-if="editing">
                                <input v-model="v.name" placeholder="Value name" class="font-bold text-sm border-gray-200 rounded w-64" />
                                <textarea v-model="v.track_question" rows="2" placeholder="TRACK question" class="w-full mt-2 text-xs border-gray-200 rounded"></textarea>
                            </template>
                            <template v-else>
                                <p class="text-sm font-black text-gray-800">{{ v.name }}</p>
                                <p class="text-xs text-gray-600 italic mt-1">{{ v.track_question }}</p>
                            </template>
                        </div>
                    ...
                    <ul class="mt-3 ml-1 space-y-1">
                        <li v-for="(q, qi) in v.guide_questions" :key="qi" class="flex items-start gap-2">
                            ...
                            <template v-if="editing">
                                <textarea v-model="v.guide_questions[qi]" rows="1" class="flex-1 text-xs border-gray-200 rounded"></textarea>
                                <button @click="v.guide_questions.splice(qi, 1)" class="text-red-400 hover:text-red-600 text-xs">✕</button>
                            </template>
                            <span v-else class="text-xs text-gray-600">{{ q }}</span>
                        </li>
                    </ul>
        ...
        <section class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-5 py-3 bg-gray-50 border-b border-gray-100">
                <h2 class="text-sm font-black uppercase tracking-wider text-gray-700">TRACK Rating Definition</h2>
            </div>
            <div class="divide-y divide-gray-100">
                <div v-for="(r, i) in form.ratings" :key="i" class="flex items-start gap-4 p-4">
                    ...
                    <template v-if="editing">
                        <input v-model="r.rating" class="w-12 text-center text-sm border-gray-200 rounded font-bold" />
                        <textarea v-model="r.description" rows="2" class="flex-1 text-xs border-gray-200 rounded"></textarea>
                    </template>
                    <p v-else class="text-xs text-gray-600 leading-relaxed">{{ r.description }}</p>
                </div>
            </div>
        </section>
    </div>

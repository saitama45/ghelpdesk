@php
    ...
    $isActualTimeRequest = ($changeRequest->request_type ?? 'schedule_change') === 'actual_time_adjustment';
    ...
    $formatActualTimeValue = function ($value, $clear = false) use ($formatDateTime) {
        return $clear ? 'Clear existing value' : $formatDateTime($value);
    };
@endphp

<div style="font-family: Arial, sans-serif; color: #111827; line-height: 1.5;">
    <h2 style="margin: 0 0 12px;">{{ $isActualTimeRequest ? 'Actual Time Adjustment Request' : 'Schedule Change Request' }}</h2>
    ...
    <table cellpadding="6" cellspacing="0" style="border-collapse: collapse; width: 100%; max-width: 640px;">
        ...
        @if ($isActualTimeRequest)
            <tr>
                <td style="font-weight: bold;">Schedule Date</td>
                <td>{{ $requested['schedule_date'] ?? '-' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Schedule Entry</td>
                <td>{{ filled($requested['schedule_store_id'] ?? null) ? ('Entry #' . $requested['schedule_store_id']) : 'Main schedule' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Actual Time In</td>
                <td>
                    {{ $formatActualTimeValue($original['actual_time_in'] ?? null) }}
                    &rarr;
                    {{ $formatActualTimeValue($requested['actual_time_in'] ?? null, (bool) ($requested['clear_time_in'] ?? false)) }}
                </td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Actual Time Out</td>
                <td>
                    {{ $formatActualTimeValue($original['actual_time_out'] ?? null) }}
                    &rarr;
                    {{ $formatActualTimeValue($requested['actual_time_out'] ?? null, (bool) ($requested['clear_time_out'] ?? false)) }}
                </td>
            </tr>
        @else
            <tr>
    ...
    <p style="margin-top: 16px;">
        Open the Scheduling page to review the {{ $isActualTimeRequest ? 'actual time adjustment' : 'schedule change' }} request.
    </p>
</div>
